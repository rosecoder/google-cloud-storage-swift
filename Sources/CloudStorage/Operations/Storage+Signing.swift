...
extension Storage {
  ...
  private func generateSignedURL(
    ...
    try await withSpan("storage-sign", ofKind: .client) { span in
      ...
      let canonicalQueryString = queryParameters.keys
        .sorted()
        .map {
          $0.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)! + "=" + queryParameters[
      ...
      let signature = try await sign(
        payload: Data(stringToSign.utf8), serviceAccount: serviceAccount
      )
      .map { String(format: "%02x", $0) }
      .joined()

      // Create URL
      return "https://" + host + path + "?" + canonicalQueryString + "&X-Goog-Signature="
        + signature
    }
  }
  ...
  private func signUsingIAM(payload: Data, serviceAccount: ServiceAccount) async throws -> Data {
    ...
    let requestBody = try encoder.encodeAsByteBuffer(
      Request(
        delegates: [name],
        payload: payload
      ), allocator: .init())
    ...
    var request = HTTPClientRequest(
      url: "https://iamcredentials.googleapis.com/v1/\(name):signBlob")
    request.method = .POST
    request.headers.add(
      name: "Authorization", value: "Bearer " + (try await authorization.accessToken()))
